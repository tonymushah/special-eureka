...
import libraryOnHoldQuery from "@mangadex/gql-docs/library/onHold";
...
type OnHoldLibraryResultConstuctorParams = {
...
export class OnHoldLibraryResult extends AbstractSearchResult<MangaListContentItemProps> {
	client: Client;
	params?: UserLibrarySectionParam;
	offset: number;
	limit: number;
	total: number;
	constructor({ data, client, params, offset, limit, total }: OnHoldLibraryResultConstuctorParams) {
		super(data);
		this.client = client;
		this.params = params;
		this.limit = limit;
...
export default async function executeSearchQuery(
	...
	if (result.data) {
		const data = result.data.library.onHold;
		res = {
	...
	if (res) {
		return new OnHoldLibraryResult({
			...res,
			client,
			params: param
		});
