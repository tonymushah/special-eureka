...
export default async function get_chapters_stats(client: Client, ids: string[], ignoreError: boolean = false): Promise<Map<string, ChapterCommentData>> {
    const res = await client.query(chaptersStatsQuery, {
        ids
    }).toPromise();
    ...
    if (res.data) {
        return reduce(res.data.statistics.chapter.list, (acc, item) => {
            if (item.comments)
                acc.set(item.id, {
