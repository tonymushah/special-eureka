...
type ReadingLibraryResultConstuctorParams = {
...
export class ReadingLibraryResult extends AbstractSearchResult<MangaListContentItemProps> {
	client: Client;
	params?: UserLibrarySectionParam;
	offset: number;
	limit: number;
	...
	constructor({
		...
		limit,
		total
	}: ReadingLibraryResultConstuctorParams) {
		super(data);
		this.client = client;
		this.params = params;
		this.limit = limit;
...
export default async function executeSearchQuery(
	...
	const result = await client
		.query(libraryReadingQuery, {
			param
		})
		.toPromise();
	if (result.data) {
		const data = result.data.library.reading;
		res = {
	...
	if (res) {
		return new ReadingLibraryResult({
			...res,
			client,
			params: param
		});
