...
import { CoverImageQuality, type UserFollowedTitlesQueryVariables } from "@mangadex/gql/graphql";
...
import { userFollowedTitlesGQL } from "@mangadex/gql-docs/title/user-followed";

type RecentlyAddedResultConstuctorParams = {
	data: MangaListContentItemProps[];
	client: Client;
	params: UserFollowedTitlesQueryVariables;
	offset: number;
	limit: number;
	total: number;
};

export class UserFollowedTitlesSearchResult extends AbstractSearchResult<MangaListContentItemProps> {
	client: Client;
	params: UserFollowedTitlesQueryVariables;
	offset: number;
	limit: number;
...
export default async function executeSearchQuery(
	...
	const result = await client
		.query(userFollowedTitlesGQL, {
			...params
		})
		.toPromise();
	if (result.data) {
		const data = result.data.follows.mangas;
		res = {
	...
	if (res) {
		return new UserFollowedTitlesSearchResult({
			...res,
			client,
			params
		});
