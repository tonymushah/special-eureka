...
use crate::{
    objects::blacklist::authors::{BlacklistedAuthorObject, BlacklistedAuthorResults},
    utils::{
        SortDirection,
...
pub struct BlacklistAuthorsArtistsQueries;

#[derive(Debug, InputObject, Clone, Default)]
pub struct BlacklistAuthorsArtistsListParam {
...
#[Object]
impl BlacklistAuthorsArtistsQueries {
    pub async fn list(
        &self,
        ctx: &async_graphql::Context<'_>,
        params: Option<BlacklistAuthorsArtistsListParam>,
    ) -> crate::Result<BlacklistedAuthorResults> {
        let app_handle = ctx.get_app_handle::<tauri::Wry>()?.clone();
        let params = params.unwrap_or_default();
        spawn_blocking(move || -> crate::Result<_> {
            use dbschema::authors_artists::dsl::*;
            use diesel::prelude::*;
            ...
            let mut boxed = authors_artists
                .select(BlacklistedAuthorObject::as_select())
                .into_boxed();
            ...
            if let Some(name) = params.author_name.as_ref().filter(|m| !m.is_empty()) {
                boxed = boxed.filter(author_name.like(name));
            }
    ...
    pub async fn get(
        ...
        ctx: &async_graphql::Context<'_>,
        id: Uuid,
    ) -> crate::Result<BlacklistedAuthorObject> {
        let app_handle = ctx.get_app_handle::<tauri::Wry>()?.clone();
        spawn_blocking(move || -> crate::Result<_> {
            use dbschema::authors_artists::dsl::*;
            use diesel::prelude::*;
            ...
            Ok(authors_artists
                .select(BlacklistedAuthorObject::as_select())
                .filter(author_id.eq(id.as_bytes()))
    ...
    pub async fn get_by_ids(
        ...
        spawn_blocking(move || -> crate::Result<_> {
            ...
            let mut connection = app_handle.blacklist_database_pool()?.get_connection()?;
            Ok(authors_artists
                .select(BlacklistedAuthorObject::as_returning())
                .filter(
                    author_id.eq_any(
                        ids.into_iter()
