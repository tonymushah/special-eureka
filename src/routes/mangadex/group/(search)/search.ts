...
export default async function executeSearchQuery(client: Client, params: ScanlationGroupListParams): Promise<AbstractSearchResult<ScanlationGroupListItemData>> {
    ...
    if (result.data) {
        ...
        return new ScanlationGroupSearchResult({
            ...
            data: data.data.map<ScanlationGroupListItemData>((e) => {
                ...
                let leader = eleader ? { id: eleader.id, name: eleader.attributes.username } : undefined;
                return {
                    id: e.id,
                    name: e.attributes.name,
                    members: e.relationships.members.length, leader
                }
            })
        })
    }
