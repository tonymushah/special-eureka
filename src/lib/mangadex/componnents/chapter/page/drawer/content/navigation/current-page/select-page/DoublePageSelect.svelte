...
<script lang="ts">
	...
	import { every, isArray } from "es-toolkit/compat";
	...
	let doublePages = $derived.by(() => images.pagesAsDoublePageIndexes);
	const currentDoublePageIndex = getChapterDoublePageCurrentPageIndex();
	let currentPageSelectedReadable = $derived.by(() => {
		const index = currentDoublePageIndex.value;
		const pages = doublePages;
	...
	const isSelected = (value: Page) => {
		...
		if (isArray(value)) {
			return every(value, (i) => {
				if (isArray(selected.value.value)) {
					return selected.value.value.includes(i);
				} else {
					return false;
				}
			});
		} else if (!isArray(selected.value.value)) {
