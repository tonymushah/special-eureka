...
type PlanToReadLibraryResultConstuctorParams = {
...
export class PlanToReadLibraryResult extends AbstractSearchResult<MangaListContentItemProps> {
	client: Client;
	...
	constructor({
		...
		limit,
		total
	}: PlanToReadLibraryResultConstuctorParams) {
		super(data);
		this.client = client;
		this.params = params;
		this.limit = limit;
...
export default async function executeSearchQuery(
	...
	const result = await client
		.query(libraryPlanToReadQuery, {
			param
		})
		.toPromise();
	if (result.data) {
		const data = result.data.library.planToRead;
		res = {
