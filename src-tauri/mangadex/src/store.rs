...
pub fn get_store_builder<R: Runtime>(app: AppHandle<R>) -> tauri::plugin::Result<StoreBuilder<R>> {
    let builder = {
        ...
        SidebarDirectionStore::default_store(ReadingModeStore::default_store(
            ReadingDirectionStore::default_store(RefreshTokenStore::default_store(
                ClientInfoStore::default_store(b)?,
            )?)?,
        )?)?
    };
    Ok(builder)
