<script lang="ts">
	...
	let hasRelation = $derived(
		data.queryResult!.relationships.manga.length > 0,
	);
	...
	let disableAddToLibrary = $derived(
		(readingStatusMutation.isPending || !$isLogged) && !dev,
	);
	...
	let shouldInfoBeneathDesc = $derived(
		mangaInfoPos == MangaInfosPositions.BeneathDescription,
	);
</script>
...
<div bind:this={isInViewPortTrigger}>
	<MangaPageTopInfo
		...
		{onrating}
		disableRating={(disableRating || !$isLogged) && !dev}
		disableRead={!hasChaptToRead.value}
		onread={() => {
			readManga(data.layoutData.id);
		}}
		disableAddToList={($isAddingToList || !$isLogged) && !dev}
		onaddToList={() => {
		...
		>
			...
			>
				{#if collapsed}
					Show more
				{:else}
					<ArrowUpFromLine size="14" /> Show less <ArrowUpFromLine
						size="14"
					/>
				{/if}
			</button>
...
<style lang="scss">
	...
	*/
	...
	@include bp.media-only-screen-breakpoint-down(
		map.get(bp.$grid-breakpoints, "xl")
	) {
		.top {
			display: block;
		}
