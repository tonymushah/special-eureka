...
#[Object]
impl MangaLinks {
    async fn has_no_links(&self) -> bool {
        self.amazon.is_none()
    ...
    async fn amazon(&self) -> Option<&Url> {
        self.amazon.as_ref()
    }
    async fn anilist(&self) -> Option<Url> {
    ...
    async fn anime_planet(&self) -> Option<Url> {
    ...
    async fn book_walker(&self) -> Option<Url> {
        let inner = self.book_walker.as_ref()?;
        Url::parse(&inner.to_string()).ok()
    }
    async fn cd_japan(&self) -> Option<Url> {
        let inner = self.cd_japan.as_ref()?;
        Url::parse(&inner).ok()
    }
    async fn ebook_japan(&self) -> Option<&Url> {
        self.ebook_japan.as_ref()
    }
    async fn english_translation(&self) -> Option<Url> {
        self.english_translation
            .as_ref()
            .and_then(|u| Url::parse(&u).ok())
    }
    async fn kitsu(&self) -> Option<Url> {
